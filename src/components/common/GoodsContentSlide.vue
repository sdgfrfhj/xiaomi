<template>
	<div class="content"
		...
		<div class="slide-wrap">
			...
			<ul class="dot-list">
				<li class="dot-item" v-for="item in 4" :class="{'active': this.currPage === $index}" @click="evtMouseEnter($index, $event)">
					<span class="dot"></span>
				</li>
			</ul>
		</div>
...
export default {
	...
	methods: {
		...
		evtMouseEnter (index, event) {
			let $li = event.currentTarget
			$li.parentNode.previousElementSibling.style.marginLeft = index * (-296) + 'px'
			this.currPage = index
			/* let cs = $li.parentElement.children
			for (let i = 0; i < cs.length; i++) {
				let csi = cs[i]
				if (csi === $li) {
					csi.classList.add('active')
					this.currPage = index
				} else {
					csi.classList.remove('active')
				}
				console.log(csi)
			}*/
		},
